    ...
    <div class="modal">
      ...
      <form @submit.prevent>
        ...
        <button class="btn" type="submit" @click="handleFormSubmit">{{isNewUser ? 'Sign Up' : 'Sign In'}}</button>
          <span v-if="isNewUser" class="form-type">No account? <b @click="setIsNewUser(false)">Create one!</b></span>
          <span v-else class="form-type">Already an user? <b @click="setIsNewUser(true)">Sign In</b></span>
      </form>
    </div>
  </div>
</template>
